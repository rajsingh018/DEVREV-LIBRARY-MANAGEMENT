...
const drawChartBook = async (subject, startIndex = 0) => {
  ...
  const cdata = await getBooks(
    `subject:${subject}&startIndex=${startIndex}&maxResults=8`
  );
  if (cdata.error) {
...
const drawListBook = async () => {
  if (searchBooks.value != "") {
    ...
    const data = await getBooks(`${searchBooks.value}&maxResults=40`);
    if (data.error) {
...
const cart = []; // Initialize an empty cart array to store book names.

function addToCart(bookName) {
    // Add the book name to the cart array.
    cart.push(bookName);
    
    // You can also provide some visual feedback to the user, such as an alert.
    alert(`"${bookName}" has been added to your cart.`);
    
    // Optionally, you can update the UI to reflect the changes in the cart.
    // For example, you can display the cart contents somewhere on the page.
    updateCartUI();
}
